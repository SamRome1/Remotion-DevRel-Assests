...
export const SupabaseCursorPluginH: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Ambient glow — left (Supabase green) */}
      <div
        style={{
          position: 'absolute',
          width: '700px',
      ...
      {/* Ambient glow — right (Cursor blue) */}
      <div
        style={{
          position: 'absolute',
          width: '700px',
      ...
      >
            ...
            {frame >= 55 && (
              <div
                style={{
                  ...
                  top: '50%',
                  height: '10px',
                  width: `${lineProgress * 110}px`,
                  transform: 'translate(-100%, -50%)',
                  background: 'linear-gradient(90deg, #3ECF8E, #00b4ff)',
                  boxShadow: '0 0 18px #3ECF8E, 0 0 36px rgba(62,207,142,0.4)',
                  imageRendering: 'pixelated',
                }}
              />
            )}
            {/* Right half of line (blue → dark blue) */}
            {frame >= 55 && (
              <div
                style={{
                  position: 'absolute',
        ...
        {frame >= 88 && (
          ...
          >
            ...
            >
              <span style={{ opacity: word1Opacity, color: '#FFD700' }}>Developers,</span>
              {' '}
              <span style={{ opacity: word2Opacity, color: '#ffffff' }}>you can now</span>
              {' '}
              <span style={{ opacity: word3Opacity, color: '#00b4ff' }}>connect to Cursor</span>
              {' '}
              <span style={{ opacity: word4Opacity, color: '#3ECF8E' }}>with a plugin</span>
            </div>
      ...
      {frame >= 80 &&
        [1, 2, 3, 4, 5, 6, 7, 8].map((i) => {
          const speed = 2 + (i % 3);
          const py = ((frame - 80 + i * 28) * speed) % 1200 - 100;
          const px = 80 + i * 240 + Math.sin(frame * 0.07 + i * 1.3) * 60;
